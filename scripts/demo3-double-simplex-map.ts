...
// print second map fitting to the first map
const square2TopLeftAngle = square1BottomLeftAngle
const square2TopRightAngle = square1BottomRightAngle
const square2BottomLeftAngle = uint16ToAngle(rand.hashUint16Int())
const square2BottomRightAngle = uint16ToAngle(rand.hashUint16Int())

const map2 = SquareMap2D.generate(
    mapSize,
    (x, y) => {
        return parsePerlinNoiseInteger(
            simplexNoise(
                x,
                y,
                square2TopLeftAngle,
                square2TopRightAngle,
                square2BottomLeftAngle,
                square2BottomRightAngle,
                mapSize,
            ),
            0,
            255,
...
console.log(drawUint8Map2D(map2.value))
